...
- Hypothesis: the page reads flat because facts are buried in prose. Give the eye a skeleton by turning the enumerable material into tables.
- Changes from the live page, and nothing else: `### Quick Facts` is now a two-column table, `### Attire & Outfits` is a two-column table, and the three `###` relationship groups are now one roster table with a `Circle` column.
- Everything else is byte-for-byte the same as the live page, so the only variable is the tables.
