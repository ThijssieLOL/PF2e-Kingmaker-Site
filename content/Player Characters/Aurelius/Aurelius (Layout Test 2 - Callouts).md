...
> An extraordinarily rare, prophetic eye condition known in ancient texts by that name, and called the "Eyes of God" informally within the Church. It sees directly into the spiritual layer of reality.
>
...
> [!todo] Grade 8 Cinder vestment
> The civilian and ceremonial vestment of [[The Vestments of the Clergy]] is still a work in progress.
...
- Hypothesis: flatness is a colour and hierarchy problem. Obsidian callouts (which Quartz renders) give bordered, tinted blocks, the closest native stand-in for a Fandom infobox and quote box.
- Callouts used: `quote` for the top quote and the references, `info` for Quick Facts and the [[Ocular of Kaelen]], `todo` for the unfinished vestment, `warning` for "The Eclipse", `example` for the casting process, `note` for meeting spirits, `danger` for the cabal.
- Tradeoff to watch: a callout is not a heading, so a callout-only block drops out of the right-hand table of contents. The top quote is now a `quote` callout rather than a plain blockquote.
