...
> **LOCAL-ONLY FILE.** This file is intentionally gitignored (see `.gitignore`). It exists
> only on Aurelius's machine and must NEVER be committed, pushed, or moved into `content/`.
> If you ever see this file appear in a `git status` or `git diff`, that is an error — restore it
> with `git checkout -- AGENTS.md` and do not commit it. The same applies to the `scripts/` folder.
...
6. **Commit** — clear, concise message; include ONLY allowed files (`git add <files>` explicitly).
   Never `git add -A` blindly. Never `git push --force`. Never rewrite shared history.
   Never revert or amend anyone else's commits.
...
- A **pre-commit hook** on this machine runs the same guard on staged files and will **block any
  commit that touches a non-editable file**. If a commit is blocked, it is a bug in your workflow —
  fix it, do NOT bypass with `--no-verify`.
...
- `AGENTS.md` (this file) and the `scripts/` folder are gitignored and local-only. You may update
  them if the user asks, but never commit them, never reference them from content notes, and never
  let them leak into `content/` (Quartz would otherwise ignore them via gitignore, but a broken
  state is still a state to avoid).
